...
export type Sportbook6vnAffixFlagCode = 'vnd' | 'usd' | 'krw' | 'gbp' | 'cad' | 'thb';

@Component({
  selector: 'sportbook6vn-affix-flag-icon',
  standalone: true,
  template: `
    @switch (code()) {
      @case ('vnd') {
        <svg class="sportbook6vn-affix-flag" viewBox="0 0 20 20" aria-hidden="true">
...
export class Sportbook6vnAffixFlagIconComponent {
  readonly code = input.required<Sportbook6vnAffixFlagCode>();
}
