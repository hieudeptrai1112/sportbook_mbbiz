...
import {
  ...
  Sportbook6vnUploadFileKind,
  Sportbook6vnUploadFileStatus,
...
export class Sportbook6vnUploadFileComponent {
  readonly label = input('Label');
  readonly required = input(true);
  readonly showLabel = input(true);
  readonly description = input<string | null>(null);
  readonly files = input<readonly Sportbook6vnUploadFileItem[]>([]);
  ...
  protected readonly inputId = computed(() => this.fileInputId() ?? `sportbook6vn-upload-file-${this.label().replace(/\s+/g, '-').toLowerCase()}`);
  ...
  protected removeFile(file: Sportbook6vnUploadFileItem, event?: Event): void {
    event?.preventDefault();
    event?.stopPropagation();
  ...
  protected trackFile(_index: number, file: Sportbook6vnUploadFileItem): string {
    return file.uid;
  }

  protected fileStatus(file: Sportbook6vnUploadFileItem): Sportbook6vnUploadFileStatus {
    return file.status ?? 'done';
  }

  protected fileKind(file: Sportbook6vnUploadFileItem): Sportbook6vnUploadFileKind {
  ...
  private normalizeFile(file: Sportbook6vnUploadFileItem): Sportbook6vnUploadFileItem {
    return {
      ...file,
  ...
  private createUploadFile(file: File): Sportbook6vnUploadFileItem {
    return {
  ...
  private maxFiles(files: Sportbook6vnUploadFileItem[]): Sportbook6vnUploadFileItem[] {
    const maxCount = this.maxCount();
    if (typeof maxCount !== 'number' || maxCount < 0) {
      return files;
    }
  ...
  private resolveFileKind(name: string): Sportbook6vnUploadFileKind {
