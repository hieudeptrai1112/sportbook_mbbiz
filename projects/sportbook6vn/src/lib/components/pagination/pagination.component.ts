...
import type {
  Sportbook6vnPaginationMode,
  Sportbook6vnPaginationPageItem,
  Sportbook6vnPaginationRange,
  Sportbook6vnPaginationRangeFormatter,
  Sportbook6vnPaginationSummaryFormatter,
...
export class Sportbook6vnPaginationComponent {
  ...
  readonly rangeFormatter = input<Sportbook6vnPaginationRangeFormatter | null>(null);
  readonly summaryFormatter = input<Sportbook6vnPaginationSummaryFormatter | null>(null);
  ...
  protected readonly range = computed<Sportbook6vnPaginationRange>(() => {
  ...
  protected readonly pageItems = computed<Sportbook6vnPaginationPageItem[]>(() => {
  ...
  protected readonly wrapperClass = computed(() =>
    [
      ...
      this.useQuickJumper() ? 'sportbook6vn-pagination--quick' : 'sportbook6vn-pagination--list',
      this.disabled() ? 'sportbook6vn-pagination--disabled' : '',
      this.jumpActive() ? 'sportbook6vn-pagination--jump-active' : '',
      this.quickInputActive() ? 'sportbook6vn-pagination--quick-input-active' : '',
      this.localOpenJump() ? 'sportbook6vn-pagination--jump-open' : '',
    ]
      .filter(Boolean)
      .join(' '),
  );
  ...
  protected trackPage(_index: number, item: Sportbook6vnPaginationPageItem): number {
    return item.page;
  }
