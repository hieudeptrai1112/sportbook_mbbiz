...
import type { MbbizMessageContent, MbbizMessageData, MbbizMessageType } from './message.types';

type MbbizNormalizedMessageType = Exclude<MbbizMessageType, 'info'>;

const DEFAULT_CONTENT: Record<Exclude<MbbizMessageType, 'info' | 'loading'>, string> = {
...
@Component({
  selector: 'mbbiz-message',
  imports: [CommonModule],
  templateUrl: './message.component.html',
  styleUrl: './message.component.scss',
})
export class MbbizMessageComponent implements OnInit, OnDestroy {
  readonly type = input<MbbizMessageType>('inform');
  readonly content = input<MbbizMessageContent | null>(null);
  ...
  readonly instance = input<MbbizMessageData | null>(null);
  ...
  protected readonly normalizedType = computed<MbbizNormalizedMessageType>(() => {
  ...
  protected readonly messageClass = computed(() =>
    [
      'mbbiz-message',
      `mbbiz-message--${this.normalizedType()}`,
      this.instance()?.options.className ?? '',
      this.shouldAnimate() ? 'mbbiz-message--animated' : '',
      this.leaving() ? 'mbbiz-message--leaving' : '',
    ]
      .filter(Boolean)
      .join(' '),
  );
