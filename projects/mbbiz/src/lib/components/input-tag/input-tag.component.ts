...
export type MbbizInputTagObjectValue = {
  value: string;
  label?: string;
  closable?: boolean;
};

export type MbbizInputTagValue = string | MbbizInputTagObjectValue;

export type MbbizInputTagRenderResult =
  | string
  | {
      label?: string;
...
export type MbbizInputTagRenderFn = (context: {
  value: string;
  label: string;
  closable: boolean;
  index: number;
  values: readonly MbbizInputTagValue[];
}) => MbbizInputTagRenderResult | null | undefined;

export type MbbizInputTagValidateResult =
  | boolean
  | string
  | MbbizInputTagObjectValue
  | Promise<boolean | string | MbbizInputTagObjectValue>;

export type MbbizInputTagValidateFn = (
  inputValue: string,
  tags: readonly MbbizInputTagValue[],
) => MbbizInputTagValidateResult;

export type MbbizInputTagMaxTagCount =
  | number
  | 'responsive'
  | {
      count: number | 'responsive';
      render?: (hiddenTagCount: number, values: readonly MbbizInputTagValue[]) => string;
    };

type MbbizInputTagModel = {
  value: string;
  label: string;
  closable: boolean;
  raw: MbbizInputTagValue;
};

@Component({
  selector: 'mbbiz-input-tag',
  imports: [NzInputModule],
  templateUrl: './input-tag.component.html',
  styleUrl: './input-tag.component.scss',
})
export class MbbizInputTagComponent implements OnDestroy {
  readonly tags = input<readonly MbbizInputTagValue[]>([]);
  ...
  readonly maxTagCount = input<MbbizInputTagMaxTagCount | null>(null);
  readonly inputId = input<string | null>(null);
  readonly validate = input<MbbizInputTagValidateFn>((inputValue, tags) => {
    return !!inputValue && tags.every((tag) => this.normalizeTag(tag).value !== inputValue);
  });
  readonly renderTag = input<MbbizInputTagRenderFn | null>(null);

  readonly valueChange = output<string>();
  readonly tagsChange = output<MbbizInputTagValue[]>();
  readonly tagRemove = output<MbbizInputTagValue>();
  ...
  protected readonly localTags = signal<MbbizInputTagModel[]>([]);
  ...
  protected readonly wrapperClass = computed(
    () =>
      [
        'mbbiz-input-tag',
        `mbbiz-input-tag--status-${this.status()}`,
        this.disabled() ? 'mbbiz-input-tag--disabled' : '',
        this.readOnly() ? 'mbbiz-input-tag--readonly' : '',
        this.hovered() ? 'mbbiz-input-tag--hovered' : '',
        this.focusState() ? 'mbbiz-input-tag--focused' : '',
        this.localTags().length > 0 ? 'mbbiz-input-tag--with-tags' : '',
        this.localTags().length > 0 && !this.localValue() ? 'mbbiz-input-tag--collapsed-input' : '',
      ]
        .filter(Boolean)
        .join(' '),
  );
  ...
  protected trackByTag(index: number, tag: MbbizInputTagModel) {
    return `${tag.value}-${index}`;
  }

  protected resolveRenderedTag(tag: MbbizInputTagModel, index: number) {
  ...
  private removeTagModel(tag: MbbizInputTagModel) {
  ...
  private normalizeTag(tag: MbbizInputTagValue): MbbizInputTagModel {
  ...
  private resolveCreatedTag(
    candidate: string,
    validateResult: boolean | string | MbbizInputTagObjectValue,
  ): MbbizInputTagValue {
  ...
  private toExternalTags(tags: readonly MbbizInputTagModel[]) {
    return tags.map((tag) => this.toExternalTag(tag));
  }

  private toExternalTag(tag: MbbizInputTagModel): MbbizInputTagValue {
  ...
  private measureResponsiveLayout() {
    ...
    const chips = Array.from(
      measureContainer.querySelectorAll<HTMLElement>('.mbbiz-input-tag__chip'),
    );
