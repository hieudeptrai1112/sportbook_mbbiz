...
import {
  MbbizDatepickerCell,
  MbbizDatepickerCellState,
  MbbizDatepickerContent,
  MbbizDatepickerField,
  MbbizDatepickerMode,
  MbbizDatepickerRangeActive,
  MbbizDatepickerRangeValue,
  MbbizDatepickerSize,
  MbbizDatepickerStatus,
...
const DEFAULT_DAY_CELLS: readonly MbbizDatepickerCell[] = [
...
const DEFAULT_INTEREST_CELLS: readonly MbbizDatepickerCell[] = Array.from({ length: 31 }, (_, index) => {
  const day = index + 1;
...
const DEFAULT_MONTH_CELLS: readonly MbbizDatepickerCell[] = MONTH_LABELS.map((label, index) => ({
  label,
  value: `2026-${`${index + 1}`.padStart(2, '0')}`,
  state: index === 4 ? 'present' : undefined,
}));

const DEFAULT_YEAR_CELLS: readonly MbbizDatepickerCell[] = [
...
const DEFAULT_TIME_CELLS: readonly MbbizDatepickerCell[] = [
...
type MbbizDatepickerTimePart = 'hour' | 'minute';

const DEFAULT_HOUR_CELLS: readonly MbbizDatepickerCell[] = Array.from({ length: 24 }, (_, index) => {
  const value = `${index}`.padStart(2, '0');
  return { label: value, value };
});

const DEFAULT_MINUTE_CELLS: readonly MbbizDatepickerCell[] = Array.from({ length: 60 }, (_, index) => {
  const value = `${index}`.padStart(2, '0');
  return { label: value, value };
});

@Component({
  selector: 'mbbiz-datepicker',
  templateUrl: './datepicker.component.html',
  styleUrl: './datepicker.component.scss',
})
export class MbbizDatepickerComponent {
  readonly mode = input<MbbizDatepickerMode>('single');
  readonly content = input<MbbizDatepickerContent>('day');
  readonly field = input<MbbizDatepickerField>('trigger');
  ...
  readonly rangeActive = input<MbbizDatepickerRangeActive>('start');
  readonly size = input<MbbizDatepickerSize>('large');
  readonly status = input<MbbizDatepickerStatus>('default');
  ...
  readonly dayCells = input<readonly MbbizDatepickerCell[]>(DEFAULT_DAY_CELLS);
  readonly interestCells = input<readonly MbbizDatepickerCell[]>(DEFAULT_INTEREST_CELLS);
  readonly monthCells = input<readonly MbbizDatepickerCell[]>(DEFAULT_MONTH_CELLS);
  readonly yearCells = input<readonly MbbizDatepickerCell[]>(DEFAULT_YEAR_CELLS);
  readonly timeCells = input<readonly MbbizDatepickerCell[]>(DEFAULT_TIME_CELLS);

  readonly valueChange = output<string | null>();
  readonly rangeChange = output<MbbizDatepickerRangeValue>();
  readonly openChange = output<boolean>();
  readonly resetClick = output<void>();
  readonly applyClick = output<MbbizDatepickerRangeValue>();
  ...
  protected readonly localRangeActive = signal<MbbizDatepickerRangeActive>('start');
  ...
  protected readonly panelContent = computed<MbbizDatepickerContent>(() =>
  ...
  protected readonly wrapperClass = computed(() =>
    [
      'mbbiz-datepicker',
      `mbbiz-datepicker--mode-${this.mode()}`,
      `mbbiz-datepicker--content-${this.content()}`,
      `mbbiz-datepicker--field-${this.field()}`,
      `mbbiz-datepicker--size-${this.size()}`,
      `mbbiz-datepicker--status-${this.status()}`,
      this.disabled() ? 'mbbiz-datepicker--disabled' : '',
      this.localOpen() ? 'mbbiz-datepicker--open' : '',
      this.showTime() ? 'mbbiz-datepicker--show-time' : '',
      this.showTimeParts() ? 'mbbiz-datepicker--split-time' : '',
    ]
      .filter(Boolean)
      .join(' '),
  );
  ...
  protected openRangePanel(part: MbbizDatepickerRangeActive) {
    if (this.disabled()) {
      return;
    }
  ...
  protected updateRangeInputValue(part: MbbizDatepickerRangeActive, event: Event) {
    const nextValue = (event.target as HTMLInputElement).value || null;
    this.localRangeActive.set(part);
  ...
  protected selectCell(cell: MbbizDatepickerCell) {
  ...
  protected selectPanelCell(cell: MbbizDatepickerCell) {
    if (this.content() === 'day' && this.dayPanelMode() === 'month') {
  ...
  protected dayCellClass(cell: MbbizDatepickerCell) {
    return this.cellClass('mbbiz-datepicker__day', cell, this.resolveDayState(cell));
  }

  protected optionCellClass(cell: MbbizDatepickerCell) {
    return this.cellClass('mbbiz-datepicker__option', cell, this.resolveOptionState(cell));
  }

  protected interestCellClass(cell: MbbizDatepickerCell) {
    return this.cellClass('mbbiz-datepicker__interest-day', cell, this.resolveInterestState(cell));
  }

  protected timeCellClass(cell: MbbizDatepickerCell) {
    return this.cellClass('mbbiz-datepicker__time', cell, this.resolveTimeState(cell));
  }

  protected timePartCellClass(part: MbbizDatepickerTimePart, cell: MbbizDatepickerCell) {
    const state = this.resolveTimePartState(part, cell);

    return [
      'mbbiz-datepicker__time',
      `mbbiz-datepicker__time--${part}`,
      `mbbiz-datepicker__time--${state}`,
      cell.disabled ? 'mbbiz-datepicker__time--disabled' : '',
    ]
      .filter(Boolean)
      .join(' ');
  }

  protected selectTimePart(part: MbbizDatepickerTimePart, cell: MbbizDatepickerCell) {
    if (this.disabled() || cell.disabled || cell.state === 'disabled') {
      return;
    }
  ...
  private selectCalendarMonth(cell: MbbizDatepickerCell) {
    if (this.disabled() || cell.disabled || cell.state === 'disabled') {
      return;
    }
  ...
  private resolveNextRangePart(): MbbizDatepickerRangeActive {
  ...
  private createTimePartCells(part: MbbizDatepickerTimePart) {
    const partIndex = part === 'hour' ? 0 : 1;
    const seen = new Set<string>();
    const cells: MbbizDatepickerCell[] = [];
  ...
  private getFallbackTimeValue(part?: MbbizDatepickerTimePart, value?: string) {
  ...
  private mergeTimePart(currentTime: string | null, part: MbbizDatepickerTimePart, value: string) {
  ...
  private resolveRangeDateValue(value: string, part: MbbizDatepickerRangeActive) {
  ...
  private createCalendarDayCells(viewDate: Date): readonly MbbizDatepickerCell[] {
    const year = viewDate.getFullYear();
  ...
  private createCalendarMonthCells(year: number): readonly MbbizDatepickerCell[] {
  ...
  private createCalendarYearCells(startYear: number): readonly MbbizDatepickerCell[] {
    const selectedValue = this.mode() === 'range' ? (this.activeStartValue() ?? this.activeEndValue()) : this.localValue();
  ...
  private createCalendarInterestCells(viewDate: Date): readonly MbbizDatepickerCell[] {
    const year = viewDate.getFullYear();
    const month = viewDate.getMonth();
    const daysInMonth = new Date(year, month + 1, 0).getDate();
  ...
  private resolveInitialCalendarDate(value: string | null, fallbackLabel: string, content: MbbizDatepickerContent) {
  ...
  private setDraftRangeValue(part: MbbizDatepickerRangeActive, value: string | null) {
    if (part === 'end') {
  ...
  private setCommittedRangeValue(part: MbbizDatepickerRangeActive, value: string | null) {
    if (part === 'end') {
  ...
  private resolveDayState(cell: MbbizDatepickerCell): MbbizDatepickerCellState {
  ...
  private resolveOptionState(cell: MbbizDatepickerCell): MbbizDatepickerCellState {
    if (cell.disabled || cell.state === 'disabled') {
      return 'disabled';
    }
  ...
  private resolveInterestState(cell: MbbizDatepickerCell): MbbizDatepickerCellState {
    if (cell.disabled || cell.state === 'disabled') {
      return 'disabled';
    }
  ...
  private resolveTimeState(cell: MbbizDatepickerCell): MbbizDatepickerCellState {
    if (cell.disabled || cell.state === 'disabled') {
      return 'disabled';
    }
  ...
  private resolveTimePartState(part: MbbizDatepickerTimePart, cell: MbbizDatepickerCell): MbbizDatepickerCellState {
    if (cell.disabled || cell.state === 'disabled') {
      return 'disabled';
    }
  ...
  private cellClass(baseClass: string, cell: MbbizDatepickerCell, state: MbbizDatepickerCellState) {
    return [
      baseClass,
      `${baseClass}--${state}`,
