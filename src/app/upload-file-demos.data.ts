...
@Component({
  ...
  template: \`
    <div class="upload-status-demo">
      <mbbiz-item-upload state="error" errorMessage="Error Message" />
      <mbbiz-item-upload state="disabled" />
    </div>
  \`,
})
export class ItemUploadStatusDemoComponent {}`,
  },
  {
    ...
    tags: ['selector=mbbiz-item-upload', 'showDataSource=true'],
...
export class ItemUploadDataSourceDemoComponent {
  ...
  },
  {
    ...
    group: 'Item File',
    descriptionParts: [
      { code: 'mbbiz-item-file' },
      { text: ' displays supported xlsx, docx, pdf, jpg, and xml file rows.' },
    ],
    tags: ['selector=mbbiz-item-file', 'status=done', 'action=remove'],
    variant: 'itemFileDefault',
    snippetTs: `import { CommonModule } from '@angular/common';
import { Component } from '@angular/core';
import { MbbizItemFileComponent, type MbbizItemFileKind } from 'mbbiz';

@Component({
  selector: 'app-item-file-default-demo',
  standalone: true,
  imports: [CommonModule, MbbizItemFileComponent],
  template: \`
    <mbbiz-item-file
...
export class ItemFileDefaultDemoComponent {
  protected readonly files: { kind: MbbizItemFileKind; name: string }[] = [
  ...
  },
  {
    ...
    tags: ['selector=mbbiz-item-file', 'showClose=false', 'showDownload=true'],
...
export class ItemFileViewDemoComponent {
  ...
  },
  {
    ...
    tags: ['selector=mbbiz-item-file', 'status=uploading', 'percent'],
...
export class ItemFileLoadingDemoComponent {
  protected readonly files: { kind: MbbizItemFileKind; name: string; percent: number }[] = [
  ...
  },
  {
    ...
    tags: ['selector=mbbiz-item-file', 'status=error', 'errorType=size/upload/format'],
...
export class ItemFileErrorDemoComponent {
  ...
  },
  {
    ...
    group: 'Upload File',
    descriptionParts: [
      { code: 'mbbiz-upload-file' },
      { text: ' renders the default upload entry with required label.' },
    ],
    tags: ['selector=mbbiz-upload-file', 'files=[]'],
    variant: 'uploadBasic',
    snippetTs: `import { Component } from '@angular/core';
import { MbbizUploadFileComponent } from 'mbbiz';
...
export class UploadFileBasicDemoComponent {}`,
  },
  {
    ...
    tags: ['selector=mbbiz-upload-file', 'expanded=false/true'],
    variant: 'uploadUploaded',
    snippetTs: `import { Component } from '@angular/core';
import { MbbizUploadFileComponent, type MbbizUploadFileItem } from 'mbbiz';
...
@Component({
  ...
  template: \`
    <div class="upload-file-stack">
      <mbbiz-upload-file [files]="collapsedFiles" [expanded]="false" />
      <mbbiz-upload-file [files]="expandedFiles" [expanded]="true" />
    </div>
  \`,
})
export class UploadFileUploadedDemoComponent {
  protected readonly collapsedFiles: MbbizUploadFileItem[] = [
  ...
  protected readonly expandedFiles: MbbizUploadFileItem[] = [
    ...this.collapsedFiles,
  ...
  },
  {
    ...
    tags: ['selector=mbbiz-upload-file', 'file.status=uploading'],
...
@Component({
  ...
  imports: [MbbizUploadFileComponent],
  template: \`
    <mbbiz-upload-file [files]="files" [expanded]="true" />
  \`,
})
export class UploadFileLoadingDemoComponent {
  protected readonly files: MbbizUploadFileItem[] = [
    {
      uid: 'pdf-loading',
  ...
  },
  {
    ...
    tags: ['selector=mbbiz-upload-file', 'errorMessage', 'file.status=error'],
...
@Component({
  ...
  template: \`
    <div class="upload-file-stack">
      <mbbiz-upload-file errorMessage="Vui lòng tải lên thông tin" />
      <mbbiz-upload-file [files]="files" [expanded]="true" />
    </div>
  \`,
})
export class UploadFileStatusDemoComponent {
  ...
  },
  {
    ...
    tags: ['selector=mbbiz-upload-file', 'disabled=true', 'maxCount=3'],
...
@Component({
  ...
  template: \`
    <div class="upload-file-stack">
      <mbbiz-upload-file [disabled]="true" />
      <mbbiz-upload-file [files]="files" [maxCount]="3" [expanded]="true" />
    </div>
  \`,
})
export class UploadFileDisabledDemoComponent {
  ...
  },
  {
    ...
    tags: ['selector=mbbiz-upload-file', 'showUploadArea=false', 'showDownload=true'],
...
export class UploadFileTypesDemoComponent {
  ...
  protected readonly downloadFile = (file: MbbizUploadFileItem): void => {
...
export const UPLOAD_FILE_ITEM_UPLOAD_API_ROWS: UploadFileApiRow[] = [
  { property: 'state', description: 'Visual state for the upload dropzone.', type: "MbbizItemUploadState", defaultValue: "'default'" },
...
export const UPLOAD_FILE_ITEM_FILE_API_ROWS: UploadFileApiRow[] = [
  ...
  { property: 'fileKind', description: 'Explicit file illustration kind. When null, the kind is resolved from name.', type: 'MbbizItemFileKind | null', defaultValue: 'null' },
  { property: 'status', description: 'File row state.', type: 'MbbizItemFileStatus', defaultValue: "'done'" },
  ...
  { property: 'errorType', description: 'Built-in error message and illustration treatment.', type: 'MbbizItemFileErrorType | null', defaultValue: 'null' },
...
export const UPLOAD_FILE_API_ROWS: UploadFileApiRow[] = [
  ...
  { property: 'files', description: 'Controlled file rows rendered above the upload entry.', type: 'readonly MbbizUploadFileItem[]', defaultValue: '[]' },
  ...
  { property: 'downloadHandler', description: 'Optional callback used before fileDownload emits.', type: '((file: MbbizUploadFileItem) => void) | null', defaultValue: 'null' },
  ...
  { property: 'filesChange', description: 'Emits the updated file list after select/drop/remove.', type: 'output<MbbizUploadFileItem[]>', defaultValue: '-' },
  { property: 'change', description: 'Emits file, fileList, and change type after select/drop/remove.', type: 'output<MbbizUploadFileChange>', defaultValue: '-' },
  { property: 'fileSelect', description: 'Emits newly selected or dropped files after mapping to upload items.', type: 'output<MbbizUploadFileItem[]>', defaultValue: '-' },
  { property: 'fileRemove', description: 'Emits the removed file item.', type: 'output<MbbizUploadFileItem>', defaultValue: '-' },
  { property: 'fileDownload', description: 'Emits the downloaded file item.', type: 'output<MbbizUploadFileItem>', defaultValue: '-' },
...
export const UPLOAD_FILE_TYPE_ROWS: UploadFileApiRow[] = [
  { property: 'MbbizItemUploadState', description: 'Upload entry state.', type: "'default' | 'error' | 'disabled'", defaultValue: '-' },
  { property: 'MbbizItemFileKind', description: 'File illustration kind.', type: "'xlsx' | 'docx' | 'pdf' | 'jpg' | 'xml' | 'file' | 'error'", defaultValue: '-' },
  { property: 'MbbizItemFileStatus', description: 'File row status.', type: "'done' | 'uploading' | 'error'", defaultValue: '-' },
  { property: 'MbbizItemFileErrorType', description: 'Built-in file error copy.', type: "'size' | 'upload' | 'format'", defaultValue: '-' },
  { property: 'MbbizUploadFileChangeType', description: 'Upload list change source.', type: "'select' | 'drop' | 'remove'", defaultValue: '-' },
  { property: 'MbbizUploadFileItem', description: 'File row item model.', type: '{ uid; name; url?; size?; sizeLabel?; fileKind?; status?; percent?; errorMessage?; errorType?; disabled?; removable?; downloadable? }', defaultValue: '-' },
  { property: 'MbbizUploadFileChange', description: 'Change event payload.', type: '{ file; fileList; type }', defaultValue: '-' },
];
...
export const UPLOAD_FILE_VARIABLE_GROUPS: UploadFileVariableGroup[] = [
  {
    ...
    rows: [
      { token: 'text/brand-primary1', value: 'blue/500', appliesTo: 'Action links and active border', notes: 'Declared in item-upload component scope.' },
      { token: 'border/brand-primary3', value: 'blue/300', appliesTo: 'Default dashed border', notes: 'Used by dropzone border.' },
      { token: 'text/secondary', value: 'darkblue/500', appliesTo: 'Guide copy and separator text', notes: 'Matches upload instruction color.' },
      { token: 'text/disable2', value: 'grayscale/500', appliesTo: 'Disabled action link', notes: 'Used when state is disabled.' },
      { token: 'background/error-primary', value: 'red/500', appliesTo: 'Error border and error icon fill', notes: 'Used when state is error.' },
      { token: 'background/error-secondary', value: 'red/400', appliesTo: 'Error message text', notes: 'Rendered below the dropzone.' },
    ],
  },
  {
    title: 'Item File Variables',
    rows: [
      { token: 'text/brand-primary1', value: 'blue/500', appliesTo: 'File name and download icon', notes: 'Primary action color.' },
      { token: 'background/secondary', value: 'darkblue/200', appliesTo: 'File row background', notes: 'Applied to the whole row.' },
      { token: 'text/primary', value: 'darkblue/1000', appliesTo: 'Base row text color', notes: 'Component text default.' },
      { token: 'text/secondary', value: 'darkblue/500', appliesTo: 'File size text', notes: 'Secondary file metadata.' },
      { token: 'background/brand-secondary1', value: 'purple/500', appliesTo: 'Progress bar fill', notes: 'Uploading state.' },
      { token: 'background/brand-secondary4', value: 'purple/200', appliesTo: 'Progress track', notes: 'Uploading state track.' },
      { token: 'background/error-secondary', value: 'red/400', appliesTo: 'Error message text', notes: 'Used when status is error.' },
    ],
  },
  {
    title: 'Upload File Wrapper Variables',
    rows: [
      { token: 'text/brand-primary1', value: 'blue/500', appliesTo: 'Expand action', notes: 'Wrapper-level action color.' },
      { token: 'text/primary', value: 'darkblue/1000', appliesTo: 'Label and base text', notes: 'Wrapper text default.' },
      { token: 'text/secondary', value: 'darkblue/500', appliesTo: 'Description text', notes: 'Optional helper copy.' },
      { token: 'background/error-secondary', value: 'red/400', appliesTo: 'Required marker', notes: 'Displayed next to label when required is true.' },
    ],
  }
];
...
export const UPLOAD_FILE_VARIABLE_NOTES = [
  ...
  'The wrapper uses native file input behavior for select/drop, then emits mapped MbbizUploadFileItem values.',
];
