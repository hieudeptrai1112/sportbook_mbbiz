...
export type Sportbook6vnCheckboxInteractionState = 'default' | 'hover' | 'pressed';

@Component({
  selector: 'sportbook6vn-checkbox',
  templateUrl: './checkbox.component.html',
  styleUrl: './checkbox.component.scss',
})
export class Sportbook6vnCheckboxComponent {
  ...
  readonly interactionState = input<Sportbook6vnCheckboxInteractionState>('default');
  ...
  protected readonly wrapperClass = computed(
    () =>
      [
        ...
        this.resolvedChecked() ? 'sportbook6vn-checkbox--checked' : '',
        this.resolvedIndeterminate() ? 'sportbook6vn-checkbox--indeterminate' : '',
        this.disabled() ? 'sportbook6vn-checkbox--disabled' : '',
        this.error() ? 'sportbook6vn-checkbox--error' : '',
        `sportbook6vn-checkbox--state-${this.interactionState()}`,
      ]
        .filter(Boolean)
        .join(' '),
  );
