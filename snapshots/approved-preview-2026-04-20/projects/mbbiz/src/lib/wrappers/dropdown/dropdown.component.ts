...
@Component({
  selector: 'mbbiz-dropdown',
  templateUrl: './dropdown.component.html',
  styleUrl: './dropdown.component.scss',
})
export class MbbizDropdownComponent implements OnDestroy {
  ...
  readonly mode = input<MbbizDropdownMode>('single');
  readonly labelMode = input<MbbizDropdownLabelMode>('outside');
  ...
  protected readonly wrapperClass = computed(
    () =>
      [
        'mbbiz-dropdown',
        `mbbiz-dropdown--label-${this.labelMode()}`,
        `mbbiz-dropdown--mode-${this.mode()}`,
        `mbbiz-dropdown--status-${this.status()}`,
        this.disabled() ? 'mbbiz-dropdown--disabled' : '',
      ]
        .filter(Boolean)
        .join(' '),
  );
  ...
  protected readonly fieldClass = computed(
    () =>
      [
        'mbbiz-dropdown__field',
        this.localOpen() ? 'mbbiz-dropdown__field--open' : '',
        this.hasSelection() ? 'mbbiz-dropdown__field--filled' : '',
        this.showSearch() ? 'mbbiz-dropdown__field--searchable' : '',
      ]
        .filter(Boolean)
        .join(' '),
  );
  ...
  protected readonly panelClass = computed(
    () =>
      [
        'mbbiz-dropdown__panel',
        this.filteredItems().length > this.searchThreshold()
          ? 'mbbiz-dropdown__panel--scrollable'
          : '',
      ]
        .filter(Boolean)
        .join(' '),
  ...
  protected readonly placeholderClass = computed(() =>
    this.hasSelection()
      ? 'mbbiz-dropdown__value mbbiz-dropdown__value--filled'
      : 'mbbiz-dropdown__value mbbiz-dropdown__value--placeholder',
  );
